...
xl_tech_regime = pd.ExcelFile(
    'Проверка на реальных данных\\Отдельное (месторожд)\\otdelnoe\\Техрежим, ОАО  Газпромнефть-Ноябрьскнефтегаз , май 2020.xlsx')
...
xl_pechat = pd.ExcelFile(
        'Проверка на реальных данных\\Отдельное (месторожд)\\otdelnoe\\Печать справочника физико-химических свойств пласта.xlsx')
...
xl_P_bh = pd.ExcelFile('Проверка на реальных данных\\Отдельное (месторожд)\\otdelnoe\\day_new (добвлен еще один столбец давления).xlsx')
...
for index_, elem in enumerate(res_prms[srez_l:srez_r]):
    ...
    if True:
        f = open(os.path.join(result_directory, 'Well num: ' +str(well_num) + ' - parameters_log.txt'), 'w')
